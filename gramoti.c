...
int makePDF (char *NameSurname, char *TeacherFIO, char *PlaceName, char *email, char *current_dir, int number)
{
    ...
    HPDF_PageDirection diploma_direction = HPDF_PAGE_LANDSCAPE;     // - page direction is LANDSCAPE
    ...
    HPDF_REAL bold_font_pt = 26, regular_font_pt = 8; // font sizes in points
    ...
    if (TeacherFIO != NULL) 
        sprintf (pedagog, "пед. %s", TeacherFIO);
    ...
    const float upperlimit = page_height - 1000.0;  //  320px from the top
    ...
    float offset = (upperlimit - lowerlimit - bold_cap - 2*regular_cap*1.2) / 4;
    ...
    box2.height = regular_cap;
        
    current_line = current_line - box1.height - offset;
    
    border.top = current_line; border.bottom = current_line - regular_cap; border.left = (page_width - box2.width) / 2; border.right = border.left + box2.width;

    draw_Centered_OneLine_Text(pdf, page, pedagog, border, regular_font, regular_font_size, 1);
    ...
    box3.height = box2.height * 6;

    current_line = current_line - box2.height - offset + 15;
