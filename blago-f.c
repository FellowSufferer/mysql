...
int makePDF (char *NameSurname, char *PlaceName, char *email, char *current_dir, int number)
{
    ...
    char *background_path = "bgs/bpf.png"; 
    
    //  End of definitions
    
    sprintf(filename, "%s/%s - %d.pdf", current_dir, NameSurname, number);
    //  printf ("%s \n", filename);
...
{  
    ...
    if (mysql_query(con, "SELECT UPPER(RTRIM(LTRIM(FIOt))), RTRIM(LTRIM(place)), LTRIM(RTRIM(email)) FROM mydb.females ORDER BY email asc")) 
    {
        fprintf(stderr, "%s\n", mysql_error(con));
        mysql_close(con);
        exit(1);
